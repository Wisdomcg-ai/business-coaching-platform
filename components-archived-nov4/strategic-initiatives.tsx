...
import { 
  Plus, X, ChevronDown, ChevronUp, Check,
...
const categoryInfo: Record<InitiativeCategory, { label: string; description: string }> = {
  attract: { label: 'Attract', description: 'Marketing and lead generation initiatives' },
  convert: { label: 'Convert', description: 'Sales and conversion optimization' },
  deliver: { label: 'Deliver', description: 'Product and service delivery excellence' },
  delight: { label: 'Delight', description: 'Customer success and retention' },
  systems: { label: 'Systems', description: 'Process and technology systems' },
  people: { label: 'People', description: 'Team and culture development' },
  profit: { label: 'Profit', description: 'Financial management and optimization' },
  strategy: { label: 'Strategy', description: 'Strategic planning and positioning' }
};

// Revenue stage definitions
...
export default function StrategicInitiatives() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load roadmap completions from localStorage
      const storedCompletions = localStorage.getItem('roadmapCompletions');
      if (storedCompletions) {
        setRoadmapCompletions(JSON.parse(storedCompletions));
      }
  ...
  const addInitiative = async (sourceType: 'custom' | 'assessment' | 'roadmap' = 'custom', suggestionType?: string) => {
    if (!newInitiative.trim() || !selectedCategory || !businessProfileId) return;
    ...
    try {
      const initiative = {
        ...
        is_from_roadmap: sourceType === 'roadmap',
        custom_source: sourceType === 'assessment' ? suggestionType : sourceType,
        selected: false,
  ...
  const addFromAssessment = async (suggestionItem: any, suggestionType: string) => {
    ...
    try {
      const initiative = {
        ...
        is_from_roadmap: false,
        custom_source: suggestionType,
        selected: false,
        quarter_assignment: null,
        order_index: initiatives.length
      };
  ...
  const toggleRoadmapCompletion = async (stage: string, category: string, itemText: string) => {
    try {
      ...
      if (existing) {
        ...
        newCompletions = roadmapCompletions.map(r => 
          r.id === existing.id 
        ...
        const newCompletion: RoadmapCompletion = {
        ...
        newCompletions = [...roadmapCompletions, newCompletion];
      }

      setRoadmapCompletions(newCompletions);
      localStorage.setItem('roadmapCompletions', JSON.stringify(newCompletions));
    } catch (error) {
  ...
  const toggleAnnualPlan = async (id: string) => {
    ...
    const selectedCount = initiatives.filter(i => i.selected).length;
    if (!initiative.selected && selectedCount >= 12) return;

    try {
      const { error } = await supabase
        .from('strategic_initiatives')
        .update({ selected: !initiative.selected })
        .eq('id', id);
                ...
                {Object.entries(categoryInfo).map(([key, info]) => {
                  ...
                  return (
                    <div key={key} className="relative group">
                      ...
                      {/* Hover Tooltip */}
                      <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-50 max-w-xs">
            ...
            {filteredInitiatives.length === 0 ? (
              ...
              <div className="space-y-2">
                {filteredInitiatives.map((initiative) => {
                  ...
                  const isDisabled = !initiative.selected && maxReached;
                  
                  // Source icon
                  const SourceIcon = initiative.is_from_roadmap ? Building : 
                                    initiative.custom_source?.includes('assessment') ? Bot : User;
                  ...
                  return (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={initiative.selected || false}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                          initiative.is_from_roadmap ? 'bg-blue-100 text-blue-700' :
                          initiative.custom_source?.includes('assessment') ? 'bg-yellow-100 text-yellow-700' :
                          'bg-green-100 text-green-700'
                        }`}>
                          <SourceIcon className="w-3 h-3" />
                          {initiative.is_from_roadmap ? 'Roadmap' :
                           initiative.custom_source?.includes('assessment') ? 'AI' :
                           'You'}
                        </div>
                      </div>
