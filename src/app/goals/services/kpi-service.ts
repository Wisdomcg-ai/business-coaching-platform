...
export class KPIService {
   ...
   */
  static async saveUserKPIs(businessId: string, kpis: KPIData[]): Promise<{ success: boolean; error?: string }> {
    try {
      if (!businessId) {
      ...
      if (deleteError) {
        console.warn('[KPI Service] ⚠️ Error deleting existing KPIs:', deleteError)
      }
      ...
      if (insertError) {
        console.error('[KPI Service] ❌ Error inserting KPIs:', insertError)
